...
class hight():
    ...
    def genVars_InVars_at_Round(self, r):
        ...
        if r > 1 :      
            A = self.genVars_InVars_at_Round(r-1)
            B = ['p'+str(j)+'Rd'+str(r-1) for j in range(32)]
            temp = A[8:16] + B[0:8] + A[24:32] + B[8:16] + A[40:48] + B[16:24] + A[56:64] + B[24:32]
            return temp
    ...
    def genConstraints_of_Round(self, r):
        ...
        constraints = constraints + ConstraintGenerator.xorConstraints(x0, m0, y7)
        ...
        for i in range(7) :
            ...
            constraints = constraints + [b[1]+' - '+a[1]+' - '+b[0]+' - '+b[2]+' + '+d[i]+' >= -2 ']
            constraints = constraints + [b[2]+' - '+a[1]+' - '+b[0]+' - '+b[1]+' + '+d[i]+' >= -2 ']
        constraints = constraints + [x2[7]+' + '+z1[7]+' + '+y1[7]+' <= 2 ']
        constraints = constraints + [x2[7]+' + '+z1[7]+' + '+y1[7]+' - 2 tpa'+str(r-1)+' >= 0 ']
        constraints = constraints + ['tpa'+str(r-1)+' - '+x2[7]+' >= 0 ']
        constraints = constraints + ['tpa'+str(r-1)+' - '+y1[7]+' >= 0 ']
        ...
        for i in range(7) :
            ...
            constraints = constraints + [b[1]+' - '+a[1]+' - '+b[0]+' - '+b[2]+' + '+d[i]+' >= -2 ']
            constraints = constraints + [b[2]+' - '+a[1]+' - '+b[0]+' - '+b[1]+' + '+d[i]+' >= -2 ']
        constraints = constraints + ConstraintGenerator.xorConstraints(x4, m1, y3)
        ...
        for i in range(7) :
            ...
            constraints = constraints + [b[1]+' - '+a[1]+' - '+b[0]+' - '+b[2]+' + '+d[i]+' >= -2 ']
            constraints = constraints + [b[2]+' - '+a[1]+' - '+b[0]+' - '+b[1]+' + '+d[i]+' >= -2 ']
        constraints = constraints + [x6[7]+' + '+z3[7]+' + '+y5[7]+' <= 2 ']
        constraints = constraints + [x6[7]+' + '+z3[7]+' + '+y5[7]+' - 2 tpb'+str(r-1)+' >= 0 ']
        constraints = constraints + ['tpb'+str(r-1)+' - '+x6[7]+' >= 0 ']
        constraints = constraints + ['tpb'+str(r-1)+' - '+y5[7]+' >= 0 ']
    ...
    def genModel(self, r):
        ...
        filename='hight-impossible-dc-'+str(r)+'.lp'
        o=open(filename,'w')
        o.write('Minimize')
        o.write('\n')
